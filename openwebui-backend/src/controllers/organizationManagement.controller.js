...
// POST /api/super-admin/organizations/seed-models - Seed initial models (Super Admin only)
export async function seedAvailableModels(req, res) {
  try {
    console.log('🌱 Seeding available models...');
    ...
    const existingCount = await AvailableModel.countDocuments();
    
    if (existingCount > 0 && !req.body.force) {
      return res.json({
        success: false,
        error: `${existingCount} models already exist. Use 'force: true' to replace them.`,
